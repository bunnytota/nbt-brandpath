...
import { setCleanUsernameAction } from '../Redux/action/auth';
...
const AzureloginScreen = ({ navigation }) => {
    const { setCleanUsername, setAzureUsername } = useAuth();
        ...
        try {
            ...
            if (result.idToken) {
                ...
                if (payload) {
                    ...
                    setAzureUsername(userEmail);

                    // await AsyncStorage.setItem('cleanUsername', cleanUsername);
                    console.log('Logged in user:', cleanUsername);
                    dispatch(setCleanUsernameAction(cleanUsername));
                }
            }
